...
//#endif
	...
	character.texID = loadTexture("../Texturas/characters/PNG/1 Pink_Monster/Pink_Monster_Walk_6.png",imgWidth,imgHeight);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (keys[GLFW_KEY_LEFT] || keys[GLFW_KEY_A]) character.position.x -= vel;
		if (keys[GLFW_KEY_RIGHT] || keys[GLFW_KEY_D]) character.position.x += vel;
		if (keys[GLFW_KEY_UP] || keys[GLFW_KEY_W]) character.position.y += vel;
		if (keys[GLFW_KEY_DOWN] || keys[GLFW_KEY_S]) character.position.y -= vel;
